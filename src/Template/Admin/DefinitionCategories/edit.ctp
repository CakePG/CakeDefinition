<?php
  $this->layout = 'admin';
  $this->assign('title', __d('CakeDefinition', 'Definition Category').'編集 - '.__d('CakeDefinition', 'Website Admin Title').' | '.__d('CakeDefinition', 'Website Title'));
...
<ol class="breadcrumb">
  ...
  <li class="breadcrumb-item active" aria-current="page"><?= __d('CakeDefinition', 'Definition Category') ?><?= __d('CakeDefinition', 'Definition Category') ?>編集</li>
</ol>

<div class="container">
  <div class="row align-items-end mb-2">
    <div class="col-md">
      <h2><?= __d('CakeDefinition', 'Definition Category') ?>編集<hr class="d-none d-md-block"></h2>
    </div>
  ...
  <div class="card admin">
    <div class="card-header">
      <nav class="nav nav-pills nav-fill">
        ...
        <?= $this->Html->link('<i class="fa fa-angle-left" aria-hidden="true"></i>詳細へ', ['action' => 'view', $definitionCategory->id]+$this->request->query, ['class' => 'btn btn-sm btn-light', 'escape' => false]) ?>
      </nav>
    </div>
    <div class="card-body">
      <?= $this->Form->create($definitionCategory, ['type' => 'file', 'templates' => 'app_form_bootstrap']); ?>
      <?php
